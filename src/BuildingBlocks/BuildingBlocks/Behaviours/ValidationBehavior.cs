...
public class ValidationBehavior<TRequest, TResponse>
    (IEnumerable<IValidator<TRequest>> Validators) // Valide edeceği nesne TRequest olan tüm IValidator'ları tutan bir enumerable
    ...
    where TRequest : ICommand<TResponse> // TRequest'in ICommand'e eşit olmasını garantiledik, Query'ler için gereksiz bir validation yapmayacak
{
    ...
    {
        var context = new ValidationContext<TRequest>(request); // TRequest için bir validasyon içeriği tanımladık
